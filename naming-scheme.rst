...
.. note::
	This mostly relates to renaming Python **binary** packages to avoid using ``python-`` prefix without a version and does not require changing the SRPM name.
...
Check the names of binary packages you are building from your SRPM, and if you use one of the following naming schemes, fix it with the instructions provided in `Changes required`_.

Common naming scheme violations
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

See the following naming schemes which are violating the current naming guidelines:
...
Add ``%package -n`` section for the Python 2 subpackage
"""""""""""""""""""""""""""""""""""""""""""""""""""""""

This will rename the binary package to ``python2-<srcname>``. Make sure to move all related runtime requirements and use `%python_provide`_ macro, which will provide both ``python-<srcname>`` and ``python2-<srcname>`` until the switch to Python 3 happens:
...
Check your Python 3 subpackage (if you build any), and make sure you are using a `%python_provide`_ macro to handle provides:
...
Replace ``%files`` section with ``%files -n``
"""""""""""""""""""""""""""""""""""""""""""""
